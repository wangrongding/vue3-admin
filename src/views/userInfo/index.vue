<script setup lang="ts">
	...
	const tempAvatar = new URL("../../assets/image/logo.png",
...
<template>
	<div class="page-container">
		<div class="item-nav-bar">
			<el-menu :default-active="'userInfo'" class="el-menu-demo" mode="horizontal" @select="changeType">
				<el-menu-item v-for="item in [
            { name: '个人信息', value: 'userInfo' },
            { name: '登录密码', value: 'password' },
            { name: '绑定手机', value: 'phone' },
          ]" style="height: 46px" :index="item.value">
					{{ item.name }}
				</el-menu-item>
			</el-menu>
		</div>

		<Form v-show="showType == 'userInfo'" :form-params="state.formParams" style="width: 1000px; margin: 0 auto"
			class="info-form">
			<template #avatar="{ itemForm }">
				<div slot="trigger">
					<img :src="state.formParams.data.headUrl || tempAvatar" style="width: 100px; height: 100px"
						alt="" />
					<el-button slot="trigger" style="display: block; margin: 0 auto" size="small" type="text"
						:loading="loading == 'upload'" @click="state.dialogForm.dialogShow = true">
						修改头像
					</el-button>
				</div>
			</template>
		...
		<Dialog :dialogForm="state.dialogForm" class="dialog">
			<template #dialogContent>
				<div class="avatar-list">
					<img v-for="item in state.imageList" :src="item.headUrl" @click="state.tempUrl = item.headUrl"
						:style="{
		          cursor: 'pointer',
		...
		<Form v-show="showType == 'password'" :form-params="state.passwordForm" style="width: 600px; margin: 0 auto"
			class="info-form" />
		<Form v-show="showType == 'phone'" :form-params="state.phoneForm" style="width: 600px; margin: 0 auto"
			class="info-form" />
	</div>
</template>
